...
""".format(prereq, q, q, prereq))
    ...
    def finalize(self):
        ...
        print('Question 0: --/6    (not autograded)')
        for q in self.questions:
          print('Question %s: %d/%d' % (q, self.points[q], self.maxes[q]))
        print('------------------')
        print('Total: %d/%d' % (sum(self.points.values()),
            sum([self.maxes[q] for q in self.questions])+6))
...
def check_digit_classification(tracker):
    ...
    tracker.add_points(0.25) # Partial credit for passing sanity checks
...
def check_lang_id(tracker):
    ...
    tracker.add_points(1.5) # Partial credit for passing sanity checks
